"""Test package for execution.py."""
import unittest
from runtime import execution
from runtime.execution import Value, Node, store_null, TYPES, NODES, Identifier, Operator, Function
...
def new_literal(value):
    return new_node(execution._LITERAL, value)
...
class Testexecution(unittest.TestCase):
    """Test cases for the execution."""

    def test_syntax_tree(self):
        """Test the syntax_tree method."""
        syntax_tree = execution.syntax_tree()
        self.assertTrue(syntax_tree is not None)
        self.assertEqual(syntax_tree.type, NODES[execution._SEQUENCE])

    def test_namespace(self):
        """Test the Namespace class."""
        ns = execution.Namespace(None)
    ...
    def test_default_namespace(self):
        ...
        ns = execution.default_namespace()
        self.assertEqual(ns.parent, None)
    ...
    def test_run_in_substitution(self):
        ...
        context = execution.default_context()
        access_node = execution.Node(access_node_type, None)
        execution.run_in_substitution(access_node, context)
        self.assertEqual(context.ns.find_identifier("x"), None)

        value_node_type = {
            "name": "Test",
            "execution": lambda n, c: execution.store_null(),
        }
        context = execution.default_context()
        value_node = execution.Node(value_node_type, None)
        value_result = execution.run_in_substitution(value_node, context)
        self.assertEqual(value_result, execution.store_null())
    ...
    def test_store_null(self):
        ...
        a = execution.store_null()
        b = execution.store_null()
    ...
    def test_sequence_node(self):
        ...
        context = execution.default_context()
        return_node = new_node(execution._RETURN, None)
        return_node.children = [true_literal]
        # empty sequence
        empty_seq = new_node(execution._SEQUENCE)
        self.assertEqual(empty_seq.eval(context), null_literal.data)
        
        # non-empty sequence
        non_seq = new_node(execution._SEQUENCE)
        ...
        ret_seq = new_node(execution._SEQUENCE)
    ...
    def test_conditional_node(self):
        ...
        bad_conditional = new_node(execution._CONDITIONAL)
        ...
        # Test correct result
        good_conditional = new_node(execution._CONDITIONAL)
    ...
    def test_branch_node(self):
        ...
        ifelifelse_branch = new_node(execution._BRANCH)
    ...
    def test_break_node(self):
        ...
        self.assertEqual(context.behaviour, execution.BEHAVIOUR_BREAK)

    def test_continue_node(self):
        """Test the continue node."""
        context = execution.default_context()
        continue_node = new_node(execution._CONTINUE)
        self.assertEqual(continue_node.eval(context), null_literal.data)
        self.assertEqual(context.behaviour, execution.BEHAVIOUR_CONTINUE)
    ...
    def test_identifier_node(self):
        ...
        ident_node = new_node(execution._IDENTIFIER, "i")
        self.assertEqual(ident_node.eval(context).data, string_literal.data.data)
        
        # Search in parent ns
        context.ns = execution.Namespace(context.ns)
        self.assertEqual(ident_node.eval(context).data, string_literal.data.data)
        
        # Identifier does not exist
        bad_node = new_node(execution._IDENTIFIER, "nope")
